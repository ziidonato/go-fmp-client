...
type BalanceSheetStatementParams struct {
	...
	Period *Period `json:"period"` // Optional: Period type (annual or quarterly)
...
func (c *Client) BalanceSheetStatement(params BalanceSheetStatementParams) ([]BalanceSheetStatementResponse, error) {
	...
	if params.Period != nil {
		urlParams["period"] = params.Period.String()
	}

	var result []BalanceSheetStatementResponse
	err := c.doRequest(c.BaseURL+"/balance-sheet-statement", urlParams, &result)
	if err != nil {
		return nil, err
	}
